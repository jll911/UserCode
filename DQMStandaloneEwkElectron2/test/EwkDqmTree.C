#define EwkDqmTree_cxx
#include "EwkDqmTree.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void EwkDqmTree::Loop()
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) 
     ...
     {
       // HACKS to keep from hanging!
       if (jentry == 6997) continue;
       if (jentry == 23990) continue;
       if (jentry == 32514) continue;
       if (jentry == 47919) continue;
       if (jentry == 48763) continue;
       if (jentry == 70033) continue;
       if (jentry == 72581) continue;
       if (jentry == 95758) continue;
       if (jentry == 131493) continue;
       if (jentry == 132921) continue;
       if (jentry == 203308) continue;
       if (jentry == 203380) continue;
       if (jentry == 221946) continue;
       if (jentry == 222844) continue;
       if (jentry == 228495) continue;
       if (jentry == 241571) continue;
...
void EwkDqmTree::InitHistograms()
{
...
void EwkDqmTree::WriteHistograms()
{
...
void EwkDqmTree::DeleteHistograms()
{
...
// void EwkDqmTree::DrawEfficiency(TString histoName, TString denomName, TString imageName, Double_t lowBins[], const int binArraySize)
// {
//   scaleFactor = 1;
...
/*TH1F * */
void
//EwkDqmTree::DrawEfficiency(TString histoName, TString denomName, TString imageName, Double_t lowBins[], const int binArraySize)
//EwkDqmTree::DrawEfficiency(TH1F * numHist, TH1F * denomHist, TH1F * effHist, /*TString imageName,*/ Double_t lowBins[], const int binArraySize)
EwkDqmTree::DrawEfficiency(TH1F * numHist, TH1F * denomHist, TString imageName, Double_t lowBins[], const int binArraySize)
{
  //  scaleFactor = 1;
  ...
  TH1F * histNew = (TH1F*) hist->Rebin(nBins,imageName,lowBins);
  //   TH1F * hist2 = dynamic_cast<TH1F*>(referenceFile->Get("ewkElecStandaloneDQM/"+histoName));
  //   TH1F * histNew2 = hist2->Rebin(nBins,"histNew2",lowBins);
  ...
  TH1F * denomNew = (TH1F*) denom->Rebin(nBins,denomHist->GetName(),lowBins);
  //   TH1F * denom2 =  dynamic_cast<TH1F*>(referenceFile->Get("ewkElecStandaloneDQM/"+denomName));
  //   TH1F * denomNew2 = denom2->Rebin(nBins,"denomNew2",lowBins);
...
//  s_1->Draw("nostack");
  ...
  SavePlot(imageName);
  outputHistoFile->WriteTObject(&gr);

  //  delete gr;
}

void EwkDqmTree::SavePlot(const char* filename)
{
  // PNG
  TString s1(filename);
  s1.Append(".png");
