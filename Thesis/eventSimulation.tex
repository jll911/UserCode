...
since the analysis does not require hadrons to 
...
\cite{LundString} 
%(shown schematically in Figure~\ref{fig:MClund})
...
\cite{TAUOLA}, 
to decay $\tau$ particles in the samples 
that included at least one $\tau$ per event.  
...
specifically dealing with production of the W and Z, 
...
select only events which could be background 
...
In particular, 
this has permitted the estimation of rates
