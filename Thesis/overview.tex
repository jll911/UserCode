...
The Standard Model will be further explained 
...
%without exchanging charge (``neutral current'').  % Wesley add, Sridhara comment
%It was first observed in 1973 
%Neutral currents were observed 
%in the Gargamelle bubble chamber at CERN 
...
%but the Z was not the only possible explanation 
%for this phenomenon.  
...
%not 
%observed directly 
first observed %at CERN
%until 
%a decade later, 
%when it was produced in proton collisions in the 
in 
%its 
decays to oppositely-charged 
electrons and  
muons % and %protons 
at the 
...
Its mass is 
%well-known enough 
known well enough % Sridhara, good call
to be used to 
...
%the decays that look like it 
the decays that may resemble it 
must be very well-studied.  
This way, if the Higgs is seen, 
%it will be known that it is definitely something new.  
it will clearly be something new.  
...
%is 
observations are % Sridhara
built on ``events.''
An event is the name for a particle interaction %,  
%particularly one % Sridhara
...
Energies at this scale are typically  measured in 
GeV, or giga-electron-volts.  
One electron-volt (eV) is the amount of energy 
an electron gains from one volt 
of potential difference.  
At the GeV level ($10^9$ eV), 
...
produce high-enough energy particles in the detector % Sridhara
to be interesting.  
%produce high-energy particles in the detector.  
...
direction of motion.  
%Only 
In hard interactions % is 
the original 
proton momentum 
is
disturbed significantly, while % much; 
...
the colliding particles. % Sridhara, another nice catch
...
than a large room in which the people are more spread out.  
...
because it is easily computed from the % Sridhara 
location of the particles' signatures in the detector.  % this too
...
Differences between particle positions 
are often given in terms of 
$\Delta\eta$ and $\Delta\phi$; 
a two-dimensional angular distance 
$\Delta R$ can also be defined as 
\[
\Delta R = \sqrt{\Delta\eta^2 + \Delta\phi^2}
\]
This is useful when requiring that particles 
