...
int main(int argc, char **argv)
{
  ...
  if ( argc == 2 )
    ZeePlotsSlim t(argv[1], -1, 0);
  else if ( argc == 3 )
    ZeePlotsSlim t(argv[1], atol(argv[2]), 0);
  else if ( argc == 4 )
    ZeePlotsSlim t(argv[1], atol(argv[2]), atol(argv[3]));
  
  return 0;
}

ZeePlotsSlim::ZeePlotsSlim(string sample, double limit, double first) : sample_(sample)
{
...
void ZeePlotsSlim::Loop(double limit, double first)
{
...
void
ZeePlotsSlim::fillRecoElectronL1Histograms(double l1match)
{
  WP wpEb, wpEe;
...
void
ZeePlotsSlim::fillRecoElectronHltHistograms(double l1match, double hltmatch)
{
  WP wpEb, wpEe;
...
void
ZeePlotsSlim::fillRecoElectronAnalysisHistograms(double l1match, double hltmatch)
{
  WP wpEb, wpEe;
...
bool
ZeePlotsSlim::foundElectronMatchToHlt( std::string trigger, double dRmatch, unsigned int which)
{
  vector<double> *hltEta = hltEle10LWEta;
  vector<double> *hltPhi = hltEle10LWPhi;
  vector<double> *hltPt = hltEle10LWPt;
...
void
ZeePlotsSlim::fillRecoElectronHistoCollection(string whichHisto, unsigned int ele)
{
...
bool
ZeePlotsSlim::applyRecoEleCut(std::string cut, const WP & wp, unsigned int which)
{ 
...
bool
ZeePlotsSlim::applyRecoEleWp95Cut(std::string cut, unsigned int which)
{ 
  if      ( cut == "NoCuts"  )       return true;
...
bool 
ZeePlotsSlim::selectRecoElectrons()
{
...
void
ZeePlotsSlim::BookAnalysisHistograms()
{
  string y = "events";
  //  hBins bin_Et         = { y, "E_{T}"              ,  10,  0.  ,  60.   };
  //  hBins bin_ScEt         = { y, "E_{T}"              ,  20,  0.  ,  100.   };
...
void 
ZeePlotsSlim::setWp(std::string givenWp, WP & wpEb, WP & wpEe)
{
...
ZeePlotsSlim::stringParser(std::string cutString)
{
...
double
ZeePlotsSlim::invMass(double mass1, double energy1, double px1, double py1, double pz1, double mass2, double energy2, double px2, double py2, double pz2) 
{
