...
#ifndef ZeePlotsSlim_h
#define ZeePlotsSlim_h
...
/*    TBranch        *b_Electronelep;   //! */
   ...
   ZeePlotsSlim(string sample, double limit, double first);
   virtual ~ZeePlotsSlim();
...
Int_t ZeePlotsSlim::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ZeePlotsSlim::LoadTree(Long64_t entry)
{
...
void ZeePlotsSlim::Init(TTree *tree)
{
...
{
  public:
    electron(ZeePlotsSlim * const trig,  unsigned int pos)
    {
      pos_    = pos;
  ...
  private:
    ...
    double dr;
    void findClosestL1(ZeePlotsSlim * const trig)
    {
...
{
 public:
  Zcand( ZeePlotsSlim * const trig,  unsigned int e1, unsigned int e2)
    {
      index1 = e1;
      index2 = e2;

      //      elec1 = math::XYZTLorentzVector(trig->recoElectronCollection.at(e1).Px(),
      elec1 = XYZTLorentzVector(trig->recoElectronCollection.at(e1).Px(),
      //      elec1 = XYZTVector(trig->recoElectronCollection.at(e1).Px(),
				trig->recoElectronCollection.at(e1).Py(),
				trig->recoElectronCollection.at(e1).Pz(),
				trig->recoElectronCollection.at(e1).ScEnergy());
      //      elec2 = math::XYZTLorentzVector(trig->recoElectronCollection.at(e2).Px(),
      elec2 = XYZTLorentzVector(trig->recoElectronCollection.at(e2).Px(),
      //      elec2 = XYZTVector(trig->recoElectronCollection.at(e2).Px(),
