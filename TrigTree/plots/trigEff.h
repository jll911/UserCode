...
public :
   ...
   trigEff(string sample, double limit);
   virtual ~trigEff();
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(bool isData, double limit);
   virtual void     BookHistograms();
   virtual Bool_t   Notify();
   bool selectRecoElectrons(double deltaRCone);
   ...
   void fillRecoElectronL1Histograms();
   void fillRecoElectronHltHistograms(double hltmatch);
...
{
  public:
    electron(trigEff * const trig,  unsigned int pos, double deltaRCone, std::set<unsigned int> usedL1)
    {
      pos_    = pos;
      ...
      findClosestL1(trig, deltaRCone, usedL1);
    }
  ...
  private:
    ...
    double dr;
    void findClosestL1(trigEff * const trig, double deltaRCone, std::set<unsigned int> usedL1)
    {
      for ( unsigned int i = 0; i < trig->l1IsoEleEnergy->size(); ++i )
      {
        if ( usedL1.find(i) != usedL1.end() ) continue;
        
        dr = deltaR(trig->l1IsoEleEta->at(i), trig->l1IsoElePhi->at(i), eta, phi);
        
        if ( dr < deltaRCone  && trig->l1IsoEleEt->at(i) > closestL1Et )
        {
            closestL1 = i;
            closestL1dr = dr;
      ...
      {
        if ( usedL1.find(1000+i) != usedL1.end() ) continue;
        
        dr = deltaR(trig->l1NonIsoEleEta->at(i), trig->l1NonIsoElePhi->at(i), eta, phi);
        
        if ( dr < deltaRCone && trig->l1NonIsoEleEt->at(i) > closestL1Et )
        {
            closestL1 = i;
            closestL1dr = dr;
